...
- 视觉真值固定为 `docs/reference-design.png` 的「暖阳果园」方向。
- 用户明确要求精细切图和高还原度；所有水果、木框、标题牌、HUD 底板与引导牌必须独立管理，不能把整张设计稿当成游戏背景伪装实现。
- 水果资源必须提供透明 PNG、统一中心锚点和可追踪的等级顺序，视觉尺寸需与 Matter 圆形碰撞体一致。
